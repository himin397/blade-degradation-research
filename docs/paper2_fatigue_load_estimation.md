...
**ステータス**: v10.3（v10.2 = 修正案②案A・B-3 統計版主体化 → v10.3 = Codex 独立レビュー（2026-07-05）指摘の反映：§4.4 の使用可能性判断を Table 8b 統計に接続、ASTM 文の指示対象明確化）
...
**どう作ったか**: NREL 5MW参照タービンのOpenFASTシミュレーション（DLC 1.2, 240ケース, ASTM E1049 Rainflow）でDELマトリクスを作成し、Senvion MM82への幾何スケーリング（λ_R = 0.651、全40条件平均で理論比との偏差−4.6%、代表条件で6.5%）により機種適合DELマトリクスを生成した。これをPenmanshiel Wind Farm（2020年通年データを持つ5台, Zenodo公開SCADA）に適用し、10分値直接計測TIに基づく月次DELを算出した。
...
- **幾何スケーリング＋翼型プロキシの限界の定量化**: 理論比との整合（全40条件平均−4.6%、代表条件6.5%）を示しつつ、モデルの主張範囲を相対比較に限定
...
OpenFASTはNRELが開発するオープンソースaeroelastic解析コードであり [1]、NREL 5MW参照タービン [2] は荷重評価研究の標準ベンチマークである。OpenFASTを含む複数の風力工学ツールはOC5プロジェクト等の第三者比較検証に参加しており、コード間の差異と実験との整合性が継続的に評価されている [16]（同プロジェクトはfloating semisubmersibleが対象で、本研究のonshore風車とは適用条件が異なる点に留意する）。Hayman (2012) はMLifeツールによるDEL算出手法（Downing & Socie (1982) のone-pass Rainflow計数、等価サイクル数正規化）を体系化した [10]。Rainflow計数法の原理はMatsuishi & Endo (1968) [13] に遡り、Downing & Socie (1982) [14] が現場データ処理向けの one-pass 実用アルゴリズム（Algorithm II）を提示した。ASTM E1049-85 [6] はRainflow計数を含むサイクル計数法を業界標準として整理した規格であり、本研究は同規格準拠のPython `rainflow` 3.2.0 [22] を使用する。ただし、荷重時系列からサイト固有の長期DELへ接続するパイプラインは標準化されておらず、TI計測精度への依存が大きい。
...
DEL比率の理論予測との誤差6.5%は、スケーリング近似（翼型プロキシ使用）と単一シードの変動の範囲内にあり、代表条件での補助確認と位置づける。相対比較への使用可能性の判断は、以下の全40条件の統計的検証（Table 8b）に基づく。
...
この統計的整合性（全40条件平均偏差−4.6%）に基づき、本プロキシモデルは月次・台間・風況条件間の相対比較に使用可能と判断する（絶対荷重値の精度保証ではない; §5.1参照）。
...
MM82プロキシのDEL比率は理論予測（R³ = 0.276）と整合する：全40条件平均0.263で偏差−4.6%（Table 8b）、代表条件（V=10 m/s, TI=14%）の0.258で6.5%（Table 8）。相対比較（月次・台間・風況条件差）には十分な精度を持つことを示唆する。
...
1. **スケーリング妥当性**: MM82プロキシモデルのDEL比率は理論予測（0.276）と整合する（全40条件平均−4.6%、代表条件6.5%）。相対比較に有効な精度を持つ
...
| # | 想定質問 | 深刻度 | 対応状況 |
|:---:|---|:---:|---|
| 1 | MM82翼型プロキシでは絶対精度に限界がある | 中 | §5.1で主張範囲を相対比較に限定済み。理論比整合（全40条件平均−4.6%・代表条件6.5%）が根拠。**v2.0で1文主張にも限定を埋め込み済み** |
...
| v10.2 | 2026-07-05 | **修正案②案A + B-3 統計版主体化**（himinさん 決定 2026-07-05）: (1) **修正案②案A**：§2 の「Downing & Socie (1982) [14] が計算アルゴリズムを体系化した」を「現場データ処理向けの one-pass 実用アルゴリズム（Algorithm II）を提示した」に修正（全10頁精読の実態に整合。論文の自己定義は simple algorithms であり「体系化」は過大表現だった）。接続文の「このアルゴリズム」を「これらのアルゴリズム」に修正（ASTM E1049-85 が整理する対象は Algorithm I/II を含む複数アルゴリズムのため）。(2) **B-3**：スケーリング誤差の提示を統計版主体に変更。「どう作ったか」・貢献・§5.1・Conclusion 1・査読論点 #1 の5箇所で「全40条件平均−4.6%（Table 8b）」を主、「代表条件（V=10 m/s, TI=14%）6.5%（Table 8）」を従とする表記に統一。§4.4 の Table 8/8b 本体は変更なし |
| v10.3 | 2026-07-05 | **Codex 独立レビュー（v10.2 適用直後）の指摘反映**（2件・いずれも採用）：(1) **中**：§4.4 で単一ケース 6.5% の直後に「この整合性から使用可能と判断」とあり統計版主体方針と衝突 → 6.5% を「代表条件での補助確認」と位置づけ直し、使用可能性の判断文を Table 8b の後へ移動して「統計的整合性（全40条件平均−4.6%）に基づき判断」に変更。(2) **低**：「これらのアルゴリズム」の指示対象が曖昧 → 「ASTM E1049-85 はRainflow計数を含むサイクル計数法を業界標準として整理した規格」に書き換え（指示語を排し、規格の実態（サイクル計数法全般の Standard Practices）に即した表現へ） |
