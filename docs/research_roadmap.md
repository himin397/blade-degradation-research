...
最終更新: 2026-03-31（**注意：本文の進捗は 2026-04-04 までの状態。それ以降の更新は反映されていない**）

---

## 下書き注記（2026-05-03 追加・要 himinさん 判断）

**このロードマップは古くなっています**。Codex 独立レビューでも指摘済み：

- 最終更新が 2026-03-31 で、現在の3論文構成（Paper 1 v9.2 / Paper 2 v9.5 / Paper 3 v5.3）と粒度がズレている
- Phase 0-5 の初期計画と 2026-04-04 までの進捗が混在している
- 末尾で参照されている `docs/integrated_research_report.md` は **存在しないファイル**（参照削除済み・下記で対応）

### 処置の選択肢（himinさん 判断）

| 選択肢 | 内容 | メリット | デメリット |
|---|---|---|---|
| **A. 最新版に更新** | Paper 1/2/3 の最新進捗、`ai_era_research_roadmap.md` への参照、研究方針本体 `paper_plan.md` との整合を取って書き直す | 浅井先生面談で使える | 更新工数が中〜大 |
| **B. 歴史的ロードマップとして明記** | 「2026-03-31 時点のスナップショット」と冒頭に明記。最新は `paper_plan.md` ＋ `ai_era_research_roadmap.md` を参照と書く | 工数小、過去経緯の記録として保持 | 参照の混乱が残る可能性 |
| **C. docs/archive/ に退避** | 履歴的価値のみと判断し、archive ディレクトリに移動 | docs 直下が整理される | 過去経緯が見つけにくくなる |

**Claude Code の所感**：選択肢 B が中庸（工数 vs 価値）。選択肢 A は面談前にやるなら価値が高いが、`paper_plan.md` の更新を優先する方が実利が大きい可能性あり。
...
<!-- 削除済み（2026-05-03）：旧記述「統合レポート：v2.1（docs/integrated_research_report.md）」は、参照先ファイルが存在しないため削除。統合パイプラインは現在 paper3_integration_pipeline.md として独立論文化されている。 -->
