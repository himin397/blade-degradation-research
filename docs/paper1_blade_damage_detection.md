...
**ステータス**: v9.10（v9.8 = Codex レビュー反映 → v9.9 = 再現パッケージ棚卸しによる §3.3/§7 の誤記修正：ultralytics 8.3.x→**8.4.33**、シード記述の精密化、学習スクリプト記述の実態整合 → v9.10 = IEC 61400-28/LifeWind 引用追加（§1・参考文献 [21]、himinさん承認 2026-07-09））
...
21. Natarajan, A.; Dimitrov, N.K.; William Peter, D.R.; Bergami, L.; Madsen, J.; Olesen, N. et al. (2020): "Demonstration of Requirements for Life Extension of Wind Turbines Beyond Their Design Life (LifeWind)" — DTU Wind Energy Report No. E-0196, EUDP Project no. 64017-05114.
...
| v9.10 | 2026-07-09 | **IEC 61400-28 / LifeWind 引用追加（himinさん承認による適用）**: §1 Introduction 第1段落末尾に、策定中の IEC 61400-28（寿命延長標準）への業界推奨が前縁エロージョンを寿命延長点検の特別重点項目として明示している旨を追加（Natarajan 2020 全110頁精読済の §8 p.106・§3.1.1 p.13 に基づく。原文に忠実に「二大重点項目の一つ（ボルト締結と並ぶ）」と表現を較正——提案文の「最重要項目」より弱く正確な側を採用）。参考文献 [21] Natarajan 2020 を新規追加。引用番号は末尾追加とし、出現順の再採番は B-4（投稿時の引用スタイル統一）で実施。提案元 = `tools/reference_audit/batch10_round1_revision_proposals.md` ボーナス節・`reference_iec61400_28_blade_erosion.md` |
