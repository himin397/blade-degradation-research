...
- フォルダ構成：`Nordtank 2017/` と `Nordtank 2018/` の2フォルダ
- 2017年画像数：303枚
- 2018年画像数：398枚
- 総画像数：701枚（論文記載と一致）
- 画像形式：JPG
- 解像度：5280 × 2970 px（4K超の高解像度）
- ファイル名：`DJI_XXXX.JPG` と `DJI_XXXX(1).JPG` のペアが存在（同一シーンの重複撮影と思われる）
- 欠損・破損ファイル：未確認（次ステップで確認予定）

**注記**：`(1)`付きファイルの扱いはデータリーク防止の観点から重要。同一番号のペアは必ず同じsplitに入れる。
