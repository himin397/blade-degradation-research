"""
dlc22_02_run_openfast.py
DLC 2.2: OpenFAST 並列実行（36 ケース）
"""
...
if __name__ == "__main__":
    all_cases = sorted(d for d in CASES_DLC22.iterdir() if d.is_dir())
    pending   = [d for d in all_cases if not (d / "case.outb").exists()]
    done      = len(all_cases) - len(pending)
    print(f"DLC 2.2 OpenFAST: {len(all_cases)} total / {done} done / {len(pending)} pending")
    ...
    outb_done = len(list(CASES_DLC22.glob("*/case.outb")))
